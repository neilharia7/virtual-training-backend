...
def build_scorm_compatible_course(id_number: int, assignment_name: str, assignment_description: str):
	...
	print(f"ruby {os.getcwd()}/ppt_to_scorm_compliant.rb {course_name} {id_number}")
	...
	print(f"ruby {os.getcwd()}/ppt_updated.rb {course_name} {id_number} {string}")
	os.system(f"ruby {os.getcwd()}/ppt_updated.rb {course_name} {id_number} {string}")
	...
	initiate_query(f"call update_assignment_details({id_number}, '{scorm_link}')")
	print("done!")


def test(id_number: int, assignment_name: str, assignment_description: str):
	# print(
	# 	f'libreoffice --headless --convert-to pdf --outdir {os.getcwd()}/course_data/ {os.getcwd()}/course_data/{assignment_name}')
	# os.system(
	# 	f'libreoffice --headless --convert-to pdf --outdir {os.getcwd()}/course_data/ {os.getcwd()}/course_data/{assignment_name}')
	...
	# with open(f'{file_path}/{id_number}_audio.json', 'w') as af:
	# 	af.write(json.dumps(audio_file_dict))
	string = "https://digitalfi.s3.ap-south-1.amazonaws.com/courses/Probability/Probability_complete.mp3"
